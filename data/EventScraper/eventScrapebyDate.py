...
# Retrieve events from Ticketmaster API based on date range
def fetch_events(city, country_code, api_key, start_date, end_date):
    ...
    while True:
        ...
        if page >= data['page']['totalPages']:  # Don't exceed the total number of pages
            break
...
def insert_events_to_db(events, connection):
    ...
    query = """
    INSERT INTO event (name, datetime, venue, city, latitude, longitude)
    VALUES %s
    """
...
def main():
    ...
    events_data = fetch_events(city, country_code, API_KEY, start_date, end_date)
